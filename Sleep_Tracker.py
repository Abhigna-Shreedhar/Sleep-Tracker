...
"""
 First register your app as a "personal app" with Fitbit Developers at
 <https://dev.fitbit.com/apps/new>. Fill out client_secret.json according
 to the guidelines in the README. You can then pull your sleep data.

 Note:
 The start date should be written in yyyy-mm-dd format
 If the start date is not specified, it will choose a date 150 days in the past.
 If the start date was more than 150 days in the past, the script  will terminate after 150 days 
 of data
"""
...
def string_to_date(date_string):
    ...
    # Converting date string YYYY-MM-DD to a date object
...
def get_access_token(refresh_token=None):
 ...
 token_uri = secret_contents['token_uri']
    # If refresh token specified, set up the request to refresh our access token
 if refresh_token:
...
def pull_sleep_data(token, start=None):
    ...
    # If there were more than 150 requests, notify user and provide the date to start on next time
    try:
